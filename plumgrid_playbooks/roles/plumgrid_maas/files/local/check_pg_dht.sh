...
#
# Returns 3 metrics:
#  - status_pg_dht_director_<name>: UP/DWON
#  - status_pg_dht_director_<name>: UP/DWON
#  - status_pg_dht_director_<name>: UP/DWON
...
for ip in ${pg_director_ips[@]}; do
  ...
  check=$(echo $pg_status | grep "DHT node at ${ip}")
  if [[ -n $check ]];then
    metrics+=("metric status_pg_dht_director_${name} string UP")
  else
    metrics+=("metric status_pg_dht_director_${name} string DOWN")
  fi
